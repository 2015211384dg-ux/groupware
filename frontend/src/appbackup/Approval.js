...
const SIDE_MENU = [
    {
        group: '결재 문서',
        items: [
            { key: 'inbox',   label: '받은 결재함', icon: '📥' },
            { key: 'my',      label: '진행/완료',   icon: '📤' },
            { key: 'draft',   label: '임시 저장',   icon: '📝' },
            { key: 'done',    label: '처리 완료',   icon: '✅' },
            { key: 'cc',      label: '참조 문서',   icon: '👁️' },
        ]
    },
];
        ...
        <div className="ap-list-wrap">
            ...
            <div className="ap-table-wrap">
                <table className="ap-table">
                    <thead>
                        <tr>
                            <th style={{width:140}}>문서번호</th>
                            <th>제목</th>
                            <th style={{width:80}}>서식</th>
                            <th style={{width:90}}>기안자</th>
                            <th style={{width:70}}>상태</th>
                            <th style={{width:80}}>일자</th>
                        </tr>
                    </thead>
                    <tbody>
                        {loading ? (
                            ...
                            <tr key={doc.id} className="ap-row" onClick={() => navigate(`/approval/documents/${doc.id}`)}>
                                <td className="ap-docnum">{doc.doc_number || '(임시저장)'}</td>
                                <td className="ap-title">{doc.title}</td>
                                <td><span className="ap-tmpl-badge">{doc.template_name || '자유'}</span></td>
...
function DashboardHome({ onBoxSelect }) {
    ...
    const cards = [
        { key:'inbox',   label:'받은 결재',  value: summary.inbox,      icon:'📥', color:'#667eea' },
        { key:'my',      label:'진행 중',    value: summary.my_pending, icon:'🔄', color:'#f6ad55' },
        { key:'done',    label:'처리 완료',  value: summary.my_approved,icon:'✅', color:'#68d391' },
        { key:'draft',   label:'임시 저장',  value: summary.my_draft,   icon:'📝', color:'#a0aec0' },
    ];
    ...
    return (
        <div className="ap-dashboard">
            ...
            {/* 문서 작성 버튼 */}
            <div className="ap-write-section">
                <button
                    className="ap-write-btn"
