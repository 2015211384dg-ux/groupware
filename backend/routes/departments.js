...
// 부서 생성
// ============================================
router.post('/', checkRole('SUPER_ADMIN', 'HR_ADMIN'), invalidateCache(/^api:.*\/api\/v1\/departments/), async (req, res) => {
    try {
        ...
        logActivity('info', `부서 생성: "${name}" (관리자: ${req.user.name})`, { userId: req.user.id, req });
...
// 부서 순서 일괄 변경
// ============================================
router.put('/reorder', checkRole('SUPER_ADMIN', 'HR_ADMIN'), invalidateCache(/^api:.*\/api\/v1\/departments/), async (req, res) => {
    try {
...
router.put('/:id', checkRole('SUPER_ADMIN', 'HR_ADMIN'), invalidateCache(/^api:.*\/api\/v1\/departments/), async (req, res) => {
    try {
        ...
        logActivity('info', `부서 수정: ID ${req.params.id} (관리자: ${req.user.name})`, { userId: req.user.id, req });
...
router.delete('/:id', checkRole('SUPER_ADMIN', 'HR_ADMIN'), invalidateCache(/^api:.*\/api\/v1\/departments/), async (req, res) => {
    try {
        ...
        logActivity('warning', `부서 삭제: ID ${deptId} (관리자: ${req.user.name})`, { userId: req.user.id, req });
